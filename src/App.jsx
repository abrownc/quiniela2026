...
// ─── EXTRAS SCORING ──────────────────────────────────────────────────────────
function calcExtrasScore(extras, officialExtras) {
  if (!officialExtras) return 0;
  let pts = 0;
  if (extras.champion && officialExtras.champion && extras.champion === officialExtras.champion) pts += 10;
  if (extras.top_scorer && officialExtras.top_scorer && extras.top_scorer === officialExtras.top_scorer) pts += 5;
  if (extras.mexico_round && officialExtras.mexico_round && extras.mexico_round === officialExtras.mexico_round) pts += 5;
  if (extras.group_winners && officialExtras.group_winners) {
    Object.keys(GROUPS).forEach(letter => {
      if (extras.group_winners[letter] && officialExtras.group_winners[letter] &&
          extras.group_winners[letter] === officialExtras.group_winners[letter]) pts += 2;
    });
  }
  return pts;
}
...
export default function App(){
  ...
  async function loadAll(){setLoading(true);await Promise.all([loadPredictions(),loadActuals(),loadExtras(),loadOfficialExtras(),loadLeaderboard()]);setLoading(false);}

  async function loadOfficialExtras(){
    const {data}=await supabase.from("official_extras").select("*").maybeSingle();
    if(data)setOfficialExtras(data);
  }
  ...
  async function loadLeaderboard(){
    const [{data:players},{data:preds},{data:results},{data:allExtras},{data:offExtras}]=await Promise.all([
      supabase.from("players").select("id,name"),
      supabase.from("predictions").select("*"),
      supabase.from("results").select("*"),
      supabase.from("extras").select("*"),
      supabase.from("official_extras").select("*").maybeSingle(),
    ]);
    ...
    const official=offExtras?.data||offExtras||null;
    if(official)setOfficialExtras(official);
    setAllPredictions(preds);
    setAllPlayers(players);
    const board=players.map(p=>{
      const myP=preds.filter(pr=>pr.player_id===p.id);
      const matchPts=myP.reduce((s,pr)=>{const a=rMap[pr.match_id];if(!a)return s;return s+(calcScore(pr.home_goals,pr.away_goals,a.home_goals,a.away_goals)||0);},0);
      const myExtras=allExtras?.find(e=>e.player_id===p.id);
      const extraPts=myExtras&&official?calcExtrasScore(myExtras,official):0;
      const pts=matchPts+extraPts;
      ...
      return{name:p.name,pts,matchPts,extraPts,exact,total:myP.length};
    }).sort((a,b)=>b.pts-a.pts||b.exact-a.exact);
    setLeaderboard(board);
  }

  const syncResults=useCallback(async()=>{},[player]);
              ...
              <div key={p.name} style={{display:"flex",alignItems:"center",gap:10,background:p.name===player.name?"#0d2137":"#161b22",border:`1px solid ${i===0?"#2d7d32":p.name===player.name?"#1c3d5c":"#21262d"}`,borderRadius:8,padding:"10px 12px",marginBottom:6}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:10,color:"#8b949e"}}>
                    {p.total} partidos · {p.exact} exactos
                    {p.extraPts>0&&<span style={{color:"#d29922"}}> · +{p.extraPts} extras</span>}
                  </div>
                </div>
                <div style={{textAlign:"right"}}>
        ...
        {tab==="admin"&&isAdmin&&(
          <div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{fontSize:11,color:"#d29922",fontWeight:700,marginBottom:8}}>⭐ Resultados oficiales de Extras</div>
              <div style={{fontSize:10,color:"#8b949e",marginBottom:10}}>Llena esto al final del torneo para calcular puntos de extras.</div>
              {[
                {key:"champion",label:"🏆 Campeón del mundo (10 pts)",options:ALL_TEAMS},
                {key:"top_scorer",label:"👟 Campeón de goleo (5 pts)",options:TOP_SCORERS},
                {key:"mexico_round",label:"🇲🇽 Hasta dónde llegó México (5 pts)",options:MEXICO_ROUNDS},
              ].map(({key,label,options})=>(
                <div key={key} style={{marginBottom:8}}>
                  <label style={{fontSize:10,color:"#8b949e",display:"block",marginBottom:3}}>{label}</label>
                  <select style={{...S.sel,fontSize:11,padding:"5px 8px"}}
                    value={officialExtras?.[key]||""}
                    onChange={async e=>{
                      const updated={...(officialExtras||{}), [key]:e.target.value};
                      await supabase.from("official_extras").upsert({id:1,...updated},{onConflict:"id"});
                      setOfficialExtras(updated);
              ...
              <div style={{marginTop:8}}>
                <label style={{fontSize:10,color:"#8b949e",display:"block",marginBottom:4}}>Ganadores reales de grupo (2 pts c/u)</label>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {Object.entries(GROUPS).map(([letter,teams])=>(
                    <div key={letter} style={{display:"flex",alignItems:"center",gap:4}}>
                      <span style={{color:"#8b949e",fontSize:10,fontWeight:700,width:20}}>G-{letter}</span>
                      <select style={{...S.sel,fontSize:10,padding:"4px 6px"}}
                        value={(officialExtras?.group_winners||{})[letter]||""}
                        onChange={async e=>{
                          const updated={...(officialExtras||{}),group_winners:{...(officialExtras?.group_winners||{}),[letter]:e.target.value}};
                          await supabase.from("official_extras").upsert({id:1,...updated},{onConflict:"id"});
                          setOfficialExtras(updated);
                          await loadLeaderboard();
                        }}>
            ...
            <div style={{borderTop:"1px solid #21262d",paddingTop:12,marginBottom:12}}>
              <div style={{fontSize:11,color:"#f85149",fontWeight:700,marginBottom:8}}>⚽ Resultados de partidos</div>
            </div>
            {MATCHES.filter(m=>m.date<=today).map(m=>{
              ...
              return(
                <div key={m.id} style={{display:"flex",alignItems:"center",gap:6,borderBottom:"1px solid #21262d",padding:"7px 0"}}>
                  ...
                  {actuals[m.id]&&<button onClick={async()=>{
                    await supabase.from("results").delete().eq("match_id",m.id);
                    await loadActuals();await loadLeaderboard();
                  }} style={{background:"none",border:"none",color:"#f85149",fontSize:11,cursor:"pointer",padding:"0 2px"}}>✕</button>}
                </div>
              );
            })}
          </div>
